...
$redirect = false;
is_logged_in('Delete Comment');

if (POST ('delete'))
{
	$commentID = validate_input_regexp_default ($_POST['commentID'], '^[0-9]+$', -1);
	if ($commentID == -1)
		art_fatal_error ('Delete Comment', 'Remove Comment', 'Invalid comment ID');
	...
	if ($_SESSION['userID'] != $comment_array['userID'])
		art_fatal_error ('Delete Comment', 'Delete Comment', 'Go away! You cannot delete other people\'s comments');
	$comment_text = escape_string ($_POST['comment_text']);
	
	mysql_query ("UPDATE comment SET status = 'deleted' WHERE commentID = $commentID");
	$redirect = true;
}
...
if ($commentID == -1)
	art_fatal_error ('Delete Comment', 'Delete Comment', 'Invalid comment ID');
...
if ($_SESSION['userID'] != $comment_array['userID'])
	art_fatal_error ('Delete Comment', 'Delete Comment', 'You cannot delete other people\'s comments');
...
art_header ('Delete Comment');
$template = new template ('delete_comment.html');
